import os
from PyQt5.QtCore import QThread, pyqtSignal, pyqtSlot, QObject, QEventLoop, QTimer
import dir_cleaner
import traceback
...
class WorkerThread(QThread):
    ...
    def custom_delete_directory(self, path, dry_run=False, trash=False, interactive=False):
        ...
        path = dir_cleaner.normalize_path(path)
        ...
        size = dir_cleaner.get_dir_size(path)
        ...
        if not dry_run:
            try:
                if trash and dir_cleaner.TRASH_SUPPORTED:
                    # Path needs to be absolute for send2trash
                    absolute_path = os.path.abspath(path)
                    dir_cleaner.send2trash.send2trash(absolute_path)
                    self.log_update.emit(f"✓ Moved to trash: {path} ({dir_cleaner.format_size(size)})")
                else:
                    dir_cleaner.shutil.rmtree(path)
                    self.log_update.emit(f"✓ Deleted: {path} ({dir_cleaner.format_size(size)})")
                return size
            except Exception as e:
                self.log_update.emit(f"✗ Failed to delete {path}: {e}")
                return 0
        else:
            self.log_update.emit(f"Would delete: {path} ({dir_cleaner.format_size(size)})")
            return size
    ...
    def run(self):
        ...
        try:
            ...
            if interactive:
                ...
                original_delete_directory = dir_cleaner.delete_directory
                # Replace with our custom function
                dir_cleaner.delete_directory = self.custom_delete_directory
            
            # Run the appropriate operation
            if self.operation == "node_modules":
                self.progress_update.emit(10)  # Show early progress to indicate we're working
                count, saved, deleted_items = dir_cleaner.delete_node_modules(
                    directory,
                    dry_run=self.kwargs.get("dry_run", False),
                    exclude=self.kwargs.get("exclude"),
                    older_than=self.kwargs.get("older_than"),
                    min_size=dir_cleaner.parse_size(self.kwargs.get("min_size", "0")) if self.kwargs.get("min_size") else 0,
                ...
                if report_path:
                    ...
                    for path, size, status in deleted_items:
                        items.append({
                            "path": path,
                            "size": dir_cleaner.format_size(size),
                            "status": status
                        })
                ...
                if count > 0:
                    self.log_update.emit(f"\nFound and processed {count} node_modules folders, saving {dir_cleaner.format_size(saved)}")
                else:
                    self.log_update.emit(f"\nNo node_modules folders found matching your criteria.")
                    
            elif self.operation == "subdirs":
                self.progress_update.emit(10)  # Show early progress to indicate we're working
                count, saved, deleted_items = dir_cleaner.delete_subdirectories(
                    directory,
                    dry_run=self.kwargs.get("dry_run", False),
                    exclude=self.kwargs.get("exclude"),
                    older_than=self.kwargs.get("older_than"),
                ...
                if count > 0:
                    self.log_update.emit(f"\nFound and processed {count} subdirs folders, saving {dir_cleaner.format_size(saved)}")
                else:
                    self.log_update.emit(f"\nNo subdirs folders found matching your criteria.")
                    
            elif self.operation == "pattern":
                self.progress_update.emit(10)  # Show early progress to indicate we're working
                pattern = self.kwargs.get("pattern")
                count, saved, deleted_items = dir_cleaner.delete_pattern_directories(
                    directory, 
                    pattern,
                    dry_run=self.kwargs.get("dry_run", False),
                    exclude=self.kwargs.get("exclude"),
                ...
                if count > 0:
                    self.log_update.emit(f"\nFound and processed {count} pattern folders, saving {dir_cleaner.format_size(saved)}")
                else:
                    self.log_update.emit(f"\nNo pattern folders found matching your criteria.")
                    
            elif self.operation == "analyze":
                self.progress_update.emit(10)  # Show early progress to indicate we're working
                depth = self.kwargs.get("depth", 3)
                results = dir_cleaner.analyze_disk_usage(directory, depth)
                self.progress_update.emit(90)  # Show progress near completion
                ...
                if count > 0:
                    self.log_update.emit(f"\nFound and processed {count} analyze folders, saving {dir_cleaner.format_size(saved)}")
                else:
                    self.log_update.emit(f"\nNo analyze folders found matching your criteria.")
                    
            elif self.operation == "discover":
                self.progress_update.emit(10)  # Show early progress to indicate we're working
                opportunities = dir_cleaner.find_cleaning_opportunities(directory)
                self.progress_update.emit(90)  # Show progress near completion
                ...
                if result["count"] > 0:
                    self.log_update.emit(f"\nFound and processed {result['count']} discover folders, saving {dir_cleaner.format_size(result['saved'])}")
                else:
                    self.log_update.emit(f"\nNo discover folders found matching your criteria.")
                    
            elif self.operation == "preset":
                self.progress_update.emit(10)  # Show early progress to indicate we're working
                preset_name = self.kwargs.get("preset_name")
                count, saved, deleted_items = dir_cleaner.run_preset(
                    preset_name, 
                    directory,
                    dry_run=self.kwargs.get("dry_run", False),
                    exclude=self.kwargs.get("exclude"),
                ...
                if count > 0:
                    self.log_update.emit(f"\nFound and processed {count} preset folders, saving {dir_cleaner.format_size(saved)}")
                else:
                    self.log_update.emit(f"\nNo preset folders found matching your criteria.")
            
            elif self.operation == "empty_dirs":
                self.progress_update.emit(10)  # Show early progress to indicate we're working
                count, saved, deleted_items = dir_cleaner.delete_empty_directories(
                    directory,
                    dry_run=self.kwargs.get("dry_run", False),
                    exclude=self.kwargs.get("exclude"),
                    older_than=self.kwargs.get("older_than"),
            ...
            if report_path:
                self.report_data["total_space"] = dir_cleaner.format_size(result["saved"])
                generated_file = dir_cleaner.generate_html_report(self.report_data, report_path)
                result["report_path"] = generated_file
            
            # Restore original delete_directory function if we replaced it
            if interactive:
                dir_cleaner.delete_directory = original_delete_directory
    ...
    # Update the scan_only method signature
        ...
        if operation == "node_modules":
            ...
            for root, dirs, _ in os.walk(directory, topdown=False):
                if 'node_modules' in dirs:
                    ...
                    node_modules_path = dir_cleaner.normalize_path(node_modules_path)
                    if dir_cleaner.should_process(node_modules_path, kwargs.get('exclude'), 
                                               kwargs.get('older_than'), 
                                               dir_cleaner.parse_size(kwargs.get("min_size", "0")) if kwargs.get("min_size") else 0):
                        size = dir_cleaner.get_dir_size(node_modules_path)
            ...
            for root, dirs, _ in os.walk(directory, topdown=True):
                ...
                for d in matching_dirs:
                    ...
                    full_path = dir_cleaner.normalize_path(full_path)
                    if dir_cleaner.should_process(full_path, kwargs.get('exclude'), 
                                              kwargs.get('older_than'), 
                                              dir_cleaner.parse_size(kwargs.get("min_size", "0")) if kwargs.get("min_size") else 0):
                        size = dir_cleaner.get_dir_size(full_path)
            ...
            for item in os.listdir(directory):
                ...
                item_path = dir_cleaner.normalize_path(item_path)
                if os.path.isdir(item_path) and dir_cleaner.should_process(item_path, kwargs.get('exclude'), 
                                                                       kwargs.get('older_than'), 
                                                                       dir_cleaner.parse_size(kwargs.get("min_size", "0")) if kwargs.get("min_size") else 0):
                    size = dir_cleaner.get_dir_size(item_path)
            ...
            empty_dirs = dir_cleaner.find_empty_directories(directory, kwargs.get('exclude'))
    ...
    # Add a method to delete selected items
    def delete_selected_items(self, items, dry_run=False, trash=False, interactive=False):
        ...
        for path, size, category in items:
            ...
            if interactive:
                ...
                result_size = dir_cleaner.delete_directory(path, dry_run, trash, interactive)
    ...
    def delete_selected_items_and_emit(self, items, **kwargs):
        ...
        try:
            ...
            if report_path:
                ...
                self.report_data["total_space"] = dir_cleaner.format_size(saved)
                generated_file = dir_cleaner.generate_html_report(self.report_data, report_path)
                result["report_path"] = generated_file
